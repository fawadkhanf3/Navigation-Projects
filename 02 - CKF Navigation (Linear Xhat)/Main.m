...
Xhat_INT{1,1} = interval(Xhat{1,1});
Xhat_LB{1,1}  = Xhat_INT{1,1}.inf;
Xhat_UB{1,1}  = Xhat_INT{1,1}.sup;
...
%         xhat(:,k) = PHI_k*xhat(:,k-1) + G_k*[gyro_wb;accel_fb];
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%
        Xhat{k,1} = cz_linear_estimator(Xhat{k-1,1},W,V,...
            z,PHI_k,G_k,[gyro_wb;accel_fb]);
        Xhat_INT{k,1} = interval(Xhat{k,1});
        Xhat_LB{k,1}  = Xhat_INT{k,1}.inf;
        Xhat_UB{k,1}  = Xhat_INT{k,1}.sup;
        %%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%
for j = 1:N
   Roll_low(j,1)    = Xhat_LB{j,1}(1)*180/pi; 
   Pitch_low(j,1)   = Xhat_LB{j,1}(2)*180/pi; 
   Yaw_low(j,1)     = Xhat_LB{j,1}(3)*180/pi; 
   Pnorth_low(j,1)  = Xhat_LB{j,1}(4); 
   Peast_low(j,1)   = Xhat_LB{j,1}(5);
   Pheight_low(j,1) = Xhat_LB{j,1}(6);
   Vnorth_low(j,1)  = Xhat_LB{j,1}(7);
   Veast_low(j,1)   = Xhat_LB{j,1}(8);
   Vdown_low(j,1)   = Xhat_LB{j,1}(9);
   
   Roll_up(j,1)    = Xhat_UB{j,1}(1)*180/pi; 
   Pitch_up(j,1)   = Xhat_UB{j,1}(2)*180/pi; 
   Yaw_up(j,1)     = Xhat_UB{j,1}(3)*180/pi; 
   Pnorth_up(j,1)  = Xhat_UB{j,1}(4); 
   Peast_up(j,1)   = Xhat_UB{j,1}(5);
   Pheight_up(j,1) = Xhat_UB{j,1}(6);
   Vnorth_up(j,1)  = Xhat_UB{j,1}(7);
   Veast_up(j,1)   = Xhat_UB{j,1}(8);
   Vdown_up(j,1)   = Xhat_UB{j,1}(9);
end
...
plot(true_time,Pheight_up,'k--');
plot(true_time,Pheight_low,'k--');
...
plot(true_time,mag([Vnorth_up(:),Veast_up(:),Vdown_up(:)]),'k--');
plot(true_time,-mag([Vnorth_low(:),Veast_low(:),Vdown_low(:)]),'k--');
...
plot(true_time,Roll_up,'k--');
plot(true_time,Roll_low,'k--');
xlabel('Time, s');ylabel('Roll Angle, deg');
title('UAV Roll');
legend('Estimated Roll','True Roll','Upper Bound','Lower Bound');

figure(5);hold on;grid on;box on
plot(true_time,estimated_pitch,'r.-');
plot(true_time,true_pitch*180/pi,'b.-');
plot(true_time,Pitch_up,'k--');
plot(true_time,Pitch_low,'k--');
xlabel('Time, s');ylabel('Pitch Angle, deg');
title('UAV Pitch');
legend('Estimated Pitch','True Pitch','Upper Bound','Lower Bound');
...
plot(true_time,wrap(Yaw_up),'k--');
plot(true_time,wrap(Yaw_low),'k--');
